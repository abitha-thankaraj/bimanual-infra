    ...
    while True:  # Use a lock instead
        if time.time() - last_sent_msg_ts.value > control_timeperiod:
            ...
            if not queue.empty():
                ...
                x = arm.set_servo_cartesian_aa(
                    move_msg.target, wait=False, relative=True, mvacc=200, speed=100)
...
if __name__ == "__main__":
    ...
    # TODO; measure control frequency for each box. COmmands should be sent at 30-250. If <30 - choppy; if>250 drop frames. Deatils in manual:
    control_frequency = 90
    ...
    # Single process access; no need for lock; Makes process faster.
    last_sent_msg_ts = mp.Value("d", time.time(), lock=False)
    # Last message sent to the message queue by the calling process
    # TODO : Implement this as mp object; use BaseManager;
    last_queued_msg = ctx_manager.CartesianMoveMessage(
        [0, 0, 0, 0, 0, 0], relative=True)

    message_queue = mp.Queue()
    moving_process = mp.Process(target=move_robot, args=(
        message_queue, last_sent_msg_ts, last_queued_msg, control_timeperiod), name="move_proc")
