...
def start_server(queue):
    ...
    while True:
        ...
        if message.startswith("start"):
            ...
            # # Relative pose : For now, only consider the position
            # for i in range(3):
            #     rel_left_pose[i] += (left_pose[i] - prev_left_pos[i])*SCALE_FACTOR
            #     rel_right_pose[i] += (right_pose[i] - prev_right_pos[i])*SCALE_FACTOR
            # print(left_pose[0], prev_left_pos[0])
            rel_left_pose[0] += (left_pose[0] - prev_left_pos[0])*SCALE_FACTOR
            # print("Relative pose left : {}".format(rel_left_pose))
            #TODO Accumulate deltas until tolerance; Then send to queue;
